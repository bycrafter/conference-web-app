...
/** Mirrors the `/search` route's permission gate - see `router/index.js`. */
const canSearchConferences = computed(() => authStore.hasPermission(PermissionCode.CONFERENCE_VIEW_ALL, PermissionCode.CONFERENCE_VIEW_LIMITED, PermissionCode.CONFERENCE_VIEW_SELF));

const profileMenu = ref();

/**
 * `POST /v1/auth/login` (`LoginResponseDto`, account-manager) only returns
 * `username`/`role`/`token`/`expiresAt` - no email or full name field exists
 * anywhere in the auth contract. ARCHITECTURAL GAP: recommend the BFF add
 * `email`/`fullName` to the login response (or a `GET /v1/auth/me`) if the
 * profile menu needs to display them; until then only username/role are shown.
 */
...
<template>
    <div class="layout-topbar">
        ...
        <div class="layout-topbar-actions">
            <div v-if="canSearchConferences" class="layout-topbar-search hidden md:block" style="width: 18rem">
                <TopbarSearch />
            </div>
            ...
            <div class="layout-topbar-menu hidden lg:block">
                <div class="layout-topbar-menu-content">
                    ...
                    <Menu id="profile-menu" ref="profileMenu" :model="profileItems" :popup="true" />
                </div>
            </div>
        </div>
    </div>
